    ...
    └── examples/mmr1.yaml                     # 训练配置（数据、超参、算法、worker）
        └── verl/trainer/main.py               # 主入口，解析 config 启动 ray
            └── verl/trainer/ray_trainer.py     # 训练主循环（episode → rollout → update）
                ...
                ├── verl/workers/reward/custom.py      # reward 调度（CustomRewardManager）
                │   └── verl/utils/reward_score/
                │       ├── openr1_rewards_batch.py    # 实际用的 reward（batch 版）
                │       │   ├── openr1_compute_score_batch_wo_LMM  # 主入口
                │       │   ├── accuracy_reward_batch_wo_LMM       # 调 r1v 判题
                │       │   ├── format_reward_batch                # 格式检查
                │       │   └── get_repetition_penalty_reward      # 重复惩罚
                │       └── r1v.py                     # 实际判题核心（多层验证）
...
ray_trainer.fit()
  │
  ├─ reward_tensor, reward_metrics = self.reward_fn(batch)        # ray_trainer.py:1292
  │
  ▼
CustomRewardManager.__call__()                                     # custom.py
  │  batch_processing=true → 走 batch_process()
  │  收集全部 response_str / ground_truth → 批量传入
  │
  ▼
openr1_compute_score_batch_wo_LMM()                                # openr1_rewards_batch.py:533
  ├─ accuracy_reward_batch_wo_LMM()   → r1v_accuracy_reward()     # r1v.py:493（实际判题）
  ├─ format_reward_batch()                                         # :23
  ├─ get_cosine_scaled_reward_batch()  → 全为 0（yaml 配置）      # :57
  └─ get_repetition_penalty_reward()                               # :108
  │
  overall = accuracy + format + 0 + repetition_penalty
  │
  ▼
reward_tensor[i, last_token] = overall                            # 只写最后一个 token
reward_metrics["accuracy"] = [...]                                # VAS learnability 用
  │
  ▼
apply_kl_penalty()    use_kl_loss=true → reward 不动，KL 进 loss
  │
  ▼
compute_advantage()   组内 z-score → 广播到每个 token             # core_algos.py:131
```

### Reward 公式

```
overall = accuracy + format + repetition_penalty     （cosine 项为 0，无效）
...
| 分量 | 权重/参数 | 来源 |
|------|---------|------|
| accuracy | 无权重，判对错 | r1v.py 多层验证，返回 1.0/0.5/0.1/0.0 |
| format | 无权重，0 或 1 | 硬编码，`<think>` + `<answer>` 各出现且仅出现一次 |
| cosine_len | 全为 0 | yaml `cos_len_reward_config` 四项均设 0.0 |
| repetition | `ngram_size=40, max_penalty=-0.5` | 硬编码，无法在 yaml 配置 |
